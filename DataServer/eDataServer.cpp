...
bool IniteDataServer()
{
	...
	if (g_DSMode == TRUE)
	{
		wsprintf(szText, "[%s] DataServer [DS: %d][MD5: %d][MainDB: %s][AccountDB: %s][EventsDB: %s][RankingDB: %s] [Season 10]", DATASERVER_VERSION, g_DataServerListPort, g_PwEncrypt, g_MuOnlineDNS, g_MeMuOnlineDNS, g_EventServerDNS, g_RankingServerDNS);
	}
	else if (!g_UseDataServer && !g_UseExDataServer && g_UseJoinServer == TRUE && !g_DSMode)
	{
		wsprintf(szText, "[%s] JoinServer [JS: %d][MD5: %d][AccountDB: %s][Season 10]", DATASERVER_VERSION, g_JoinServerListPort, g_PwEncrypt, g_MeMuOnlineDNS);
	}
	else if (!g_UseDataServer && g_UseExDataServer == TRUE && !g_UseJoinServer && !g_DSMode)
	{
		wsprintf(szText, "[%s] ExDataServer [ExDS: %d][MD5: %d][MainDB: %s][AccountDB: %s][EventsDB: %s][RankingDB: %s] [Season 10]", DATASERVER_VERSION, g_ExDataServerListPort, g_PwEncrypt, g_MuOnlineDNS, g_MeMuOnlineDNS, g_EventServerDNS, g_RankingServerDNS);
	}
	else
	{
		wsprintf(szText, "[%s] DataServer(e) [DS: %d][ExDS: %d][JS: %d][MD5: %d][MainDB: %s][AccountDB: %s][EventsDB: %s][RankingDB: %s] [Season 10]", DATASERVER_VERSION, g_DataServerListPort, g_ExDataServerListPort, g_JoinServerListPort, g_PwEncrypt, g_MuOnlineDNS, g_MeMuOnlineDNS, g_EventServerDNS, g_RankingServerDNS);
	}
