...
void CAuthSystem::ActivationAnswer(PMSG_LICANS * pMsg)
{
	...
	if(WLRegSmartKeyCheck(pMsg->CustomerName, pMsg->ServerName, NULL, pMsg->Key) == TRUE)
	{
		...
		MessageBox(0, "Activation Completed: Click OK to close and run Data Server first.", "Success", MB_OK|MB_ICONINFORMATION);
		//ExitProcess(0);
	}
...
void CAuthSystem::SetAddons(DWORD Addons)
{
	...
	{
		MessageBoxA(0,"Your license does not have Offlevel plugin purchased, please disable OffLevel system","WARNING",0);
		ExitProcess(101);
	}
