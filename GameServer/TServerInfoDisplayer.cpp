...
{
	"[- MUEMU SERVER X12 -]" //"[-  Common  -]" or "]+[  Castle  ]+["
};

static char * ServerTypeTextVersion[1] =
{
	"[- Update - 12.0.0.30 -]" //"[-  Common  -]" or "]+[  Castle  ]+["
};
...
{
	this->m_hFont = CreateFont(50, 0, 0, 0, FW_LIGHT, FALSE, TRUE, FALSE, 
...
void TServerInfoDisplayer::PaintAllInfo(HWND hWnd, int iTopLeftX, int iTopLeftY)
{
	...
	{
		SetTextColor(hDC, RGB(0, 0, 0));
		TextOut(hDC, 50, 5, ServerErrorText[0], strlen(ServerErrorText[0]));
	}

	else if ( g_ConfigRead.server.GetServerType() == SERVER_CASTLE && g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE )
	{
		SetTextColor(hDC, RGB(255, 255, 255));
		TextOut(hDC, 50, 5, ServerCSText[0], strlen(ServerCSText[0]));
	}

	else if ( g_Crywolf.GetCrywolfState() != CRYWOLF_STATE_NONE )
	{
		SetTextColor(hDC, RGB(255, 255, 255));
		TextOut(hDC, 50, 5, ServerCWText[0], strlen(ServerCWText[0]));
	}

	else
	{
		TextOutA(hDC, 50, 5, ServerTypeText[0], strlen(ServerTypeText[0]));
